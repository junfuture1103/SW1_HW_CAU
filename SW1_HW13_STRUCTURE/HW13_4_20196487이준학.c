...
void rank_sort(rank list[], int index) {
	int min_index;
	rank temp;
	...
	for (int i = 0; i < index - 1; i++) {
		min_index = i;
		for (int j = i + 1; j < index; j++) {
			if (list[min_index].count <= list[j].count) {
				if (list[min_index].count == list[j].count) {
					if (strcmp(list[min_index].name, list[j].name) == 1) {
						min_index = j;
					}
				}
				else {
...
int main() {
	...
	for (int i = 0; i < temp_index; i++) {
		...
		stu_rank[i].count = mj_count[i];
		strncpy(stu_rank[i].name, temp[i], sizeof(temp[i]));
	}

	rank_sort(stu_rank, temp_index);

	printf("\n======== 학생 순위 출력 ========\n\n");
	for (int i = 0; i < temp_index; i++) {
		printf("%s:%d ", stu_rank[i].name, stu_rank[i].count);
	}
